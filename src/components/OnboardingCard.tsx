...
export function OnboardingCard({ onboarding }: { onboarding: Onboarding }) {
  ...
  return (
    <section className="card p-6">
      ...
      {/* No telemóvel o botão vai para baixo do texto. Ao lado, espremia a
          descrição numa coluna de meia dúzia de palavras por linha, e um passo
          que se lê mal não convida a dar passo nenhum. */}
          ...
          <li key={s.id} className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1 sm:flex sm:items-start sm:justify-between sm:gap-3">
              ...
              {!s.done ? (
                <Link
                  href={s.href}
                  className={`mt-2 inline-block shrink-0 text-xs sm:mt-0 ${
                    next?.id === s.id ? "btn-secondary" : "btn-ghost"
                  }`}
                >
                  {s.cta}
                </Link>
